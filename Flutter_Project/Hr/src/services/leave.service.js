...
const getLeaveDaysInMonth = async (employeeId, year, month) => {
  ...
  for (const leave of leaves) {
    const overlapStart = new Date(Math.max(leave.startDate.getTime(), start.getTime()));
    const overlapEnd = new Date(Math.min(leave.endDate.getTime(), end.getTime()));
